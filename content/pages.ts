export const homeCopy = {
  h1: "Tour the listing first. Visit only the finalists.",
  lede: "Virtual-first touring is the accommodation. We pre-verify access features against a published glossary, then spend in-person time on two or three properties—not twelve.",
  audienceLabel: "Who this practice is built for",
  audiences: [
    {
      title: "Buyers whose bodies constrain the search",
      body: "Wheelchair users, post-stroke and post-surgical recovery, ALS and MS, and low vision. The work is remote touring and measured features, not a motivational speech.",
    },
    {
      title: "Aging-in-place downsizers",
      body: "Sun City Summerlin, Sun City Anthem, Solera, Siena, and Del Webb searches, started before stairs become the deciding room in the house.",
    },
    {
      title: "Disabled veterans",
      body: "Nevada has a large veteran population. VA Specially Adapted Housing (SAH) and Special Housing Adaptation (SHA) grants are poorly explained by most purchase teams. We sequence the housing search; we do not quote unverified grant amounts.",
    },
  ],
  promiseTitle: "The reduced-showing promise",
  promiseBody:
    "A hospital discharge planner, an occupational therapist, a VA loan officer, or an elder law attorney should be able to send a client here and see the process in writing. No IDX grid. No overlay widget. Feature language, not persona language.",
};

export const howItWorksCopy = {
  h1: "The reduced-showing process",
  lede: "Each step exists to spend the client's physical energy only on listings that already passed a documented check.",
};

export const featuresCopy = {
  h1: "Accessibility feature glossary",
  lede: "This is the copy standard for listing notes. Features are measured. People are not described. Where a figure comes from the 2010 ADA Standards, the citation is on the row.",
};

export const veteransCopy = {
  h1: "VA SAH and SHA grants, paired with a purchase",
  lede: "Specially Adapted Housing (SAH) and Special Housing Adaptation (SHA) are VA grant programs. This page explains how a purchase search can run beside those programs. It is not a benefits determination.",
  sections: [
    {
      title: "What we will not do on this page",
      body: "PLACEHOLDER: grant dollar amounts, eligibility grids, and remaining entitlement pending source-verified VA materials. Do not treat any figure on a third-party blog as current.",
    },
    {
      title: "What the housing search can do in parallel",
      body: "Remote tours and feature sheets can start while a lender or a VA-accredited representative works the grant file. The point is to avoid spending a showing day on a two-story plan that cannot take the adaptation the grant is meant to fund.",
    },
    {
      title: "Language we use in files",
      body: "Door clear width, threshold height, shower curb, route width. Not diagnoses. Not claims about who the house is for.",
    },
  ],
};

export const agingInPlaceCopy = {
  h1: "Aging in place, Las Vegas and Henderson",
  lede: "The search is for a floor plan that still works when stairs, tub walls, and round knobs start to cost more energy than they used to.",
  communitiesTitle: "Communities we check often",
  communities: [
    {
      name: "Sun City Summerlin",
      note: "Northwest Las Vegas. Inventory mix includes single-level plans; we still measure doors and showers rather than trust the community label.",
    },
    {
      name: "Sun City Anthem",
      note: "Henderson. Same rule: 55+ is an age policy, not an access certificate.",
    },
    {
      name: "Solera",
      note: "Henderson. PLACEHOLDER: current inventory notes pending a sourced market snapshot.",
    },
    {
      name: "Siena",
      note: "Henderson. Guard-gated. Access still depends on the unit, not the gate.",
    },
    {
      name: "Del Webb communities",
      note: "Multiple Las Vegas Valley locations. Builder marketing is not a substitute for a tape measure on the unit you will actually occupy.",
    },
  ],
  universalDesign:
    "Universal design on this site means features that reduce the number of required body movements for daily tasks: zero-step entry, lever hardware, a roll-in or transferable shower, and a single finished level for sleep, food, and bathing. We record those items. We do not assign them to a type of person.",
};

export const aboutCopy = {
  h1: "PLACEHOLDER: Dr. Gene Boyle",
  lede: "Berkshire Hathaway HomeServices Nevada Properties. Las Vegas and Henderson. Virtual-first touring for buyers and sellers whose search is constrained by the body they have to live in.",
  body: [
    "PLACEHOLDER: credentials, education, and years in practice pending source documents.",
    "PLACEHOLDER: biography. Do not invent military service, medical licenses, or awards.",
    "The practice is built so a referral source can read the process, the glossary, and the Fair Housing rule (features, not people) without a sales call.",
  ],
};

export const accessibilityStatementCopy = {
  h1: "Accessibility statement",
  lede: "PLACEHOLDER: full accessibility statement pending legal review. The paragraphs below are a scaffold, not approved legal copy.",
  sections: [
    {
      title: "Commitment",
      body: "PLACEHOLDER: statement of conformance target (WCAG 2.2 Level AA) and review date.",
    },
    {
      title: "Known limits",
      body: "PLACEHOLDER: third-party content, mapped media, and any exceptions once legal has reviewed them.",
    },
    {
      title: "How to report a barrier",
      body: "Use the contact form and describe the page, the browser or assistive technology, and the task that failed. PLACEHOLDER: dedicated accessibility email and response-time commitment pending operations.",
    },
    {
      title: "What this site does not use",
      body: "No accessibility overlay widgets (including accessiBe, UserWay, and AudioEye). Overlays are not a substitute for semantic HTML.",
    },
  ],
};

export const referralPartnersCopy = {
  h1: "Referral partners",
  lede: "For people whose job is to send a client to a housing process they can inspect in writing. Read the process, keep the recordings, and leave medical and legal decisions in your lane.",
};

export const contactCopy = {
  h1: "Request a consultation",
  lede: "Tell us whether you are buying, selling, or referring. We will not ask you to tour twelve houses to prove you are serious.",
  stubNotice:
    "The form validates in the browser. Submissions are not sent until a backend is connected (PLACEHOLDER: endpoint).",
};
